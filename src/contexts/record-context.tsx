  ...
  const downloadAttachment = async (attachment: EncryptedAttachmentDTO, useCache = true) => {
    try {
      ...
      if ((isIOS() && process.env.NEXT_PUBLIC_OPTIONAL_CONVERT_PDF_SERVERSIDE) || process.env.NEXT_PUBLIC_CONVERT_PDF_SERVERSIDE) {
        ...
        url =  '/enclave/download/' + attachment.storageKey + '?encr=' + encodeURIComponent(tempKey.encryptedKey) + '&token=' + encodeURIComponent(dbContextRef.current?.accessToken) + '&klh=' + encodeURIComponent(tempKey.keyLocatorHash) + '&kh=' + encodeURIComponent(tempKey.keyHash);
      } else {
